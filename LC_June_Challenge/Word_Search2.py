...
def findWords(board: [[str]], words: [str]) -> [str]:
	...
	def search(i:int, j:int, word:str, idx:int,visitedCell:set)->bool:
		if idx == len(word):
			# print("This is the end here")
			return True # indicating that we've found all chars till now

		if i > n-1 or i < 0 or j > m-1 or j < 0 or ((i,j) in visitedCell): 
			# if (i,j) in visitedCell:
			# 	visitedCell.remove((i,j))
			# 	print(word, word[idx], (i, j), " removed")
			return False 
		flag = False
		if board[i][j] == word[idx]:
			# do bfs
			visitedCell.add((i,j))
			# print(word, word[idx], visitedCell)
			flag = search(i+1, j, word, idx+1, visitedCell) or search(i-1, j, word, idx+1, visitedCell)\
				or search(i, j+1, word, idx+1, visitedCell) or search(i, j-1, word, idx+1, visitedCell) 
			# print('Remocing the root of this recursive stack if path not found:, ', word[idx],(i, j))
			if not flag: visited.remove((i,j))
		return flag
...
board = [['a','b', 'c', 'd'], ['t', 'u', 'a', 'n'], ['i', 's', 'n', 't'], ['m','y','s','s']]
words = ['cans', 'cant', 'busy', 'tim', 'isnt', 'now']
